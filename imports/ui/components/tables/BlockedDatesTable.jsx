...
class BlockedDatesTable extends React.Component {
    constructor(props) {
        super(props);
    ...
    handleBlockedDateEdit(event, row) {
    ...
    blockedDateFormatter(cell, row) {
        return <Link to='#' onClick={e => this.handleBlockedDateEdit(e, row)}>{cell}</Link>;
    }

    dateContolsFormatter(cell, row) {
        return (
            <div style={{ textAlign: 'center' }}>
                <Link to='#' onClick={e => this.handleBlockedDateEdit(e, row)}>Edit</Link>
        ...
        return (
            <Col sm={8} className='custom-table'>
                <BootstrapTable hover tableHeaderClass='table-header' tableBodyClass='table-body' data={this.props.data}>
                    <TableHeaderColumn dataField='name' dataFormat={this.blockedDateFormatter}>Blocked date</TableHeaderColumn>
